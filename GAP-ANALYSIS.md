...
### P2 — 企业级硬要求(预计 4-6 周)
- [x] **数据校验层 + 乐观锁** ✅(2026-07-06):Pydantic 校验上收到入口 —— 金额字段
      `ge=0`(拒负数)、fund_status 枚举校验、名称非空 + 长度上限(project/fund 的
      create/update 全覆盖),非法输入自动 422。**乐观锁**:detail 编辑携带加载时
      `expected_updated_at`,与库中 updated_at 不一致 → 409(有人先改了),防并发覆盖;
      前端保存后自动重取版本。**实测:负数/非法枚举/空名 → 422;过期时间戳 PATCH → 409,
      正确 → 200。**
